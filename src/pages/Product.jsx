...
const Product = () => {
  ...
  return productData? (
    ...
    {/*product images */}
    <div className='flex-1 w-300px flex flex-col-reverse gap-3 sm:flex-row'>
    <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal w-full'>
      {
        productData.image.map((item,index)=>(
          <img  onClick={()=>setImage(item)} src={item} key={index} className=' w-[24%] sm:w-1/3 sm:mb-3 flex-shrink-0 cursor-pointer' alt=""/>
        ))
      }
    </div>
